...
function defaultFronts(): FrontsConfig {
  return { type: 'none', overlayMm: 2, gapMm: 2, handleType: 'none', doorOption: 'single', drawersCount: 1, drawersEqualHeight: true }
}

function newCabinet(projectId: string): Cabinet {
  const now = Date.now()
  return {
    id: crypto.randomUUID(), projectId, name: 'Meuble bas',
    variant: 'base', widthMm: 600, depthMm: 560, heightMm: 720,
    panelThicknessMm: 18, shelvesCount: 1, panelType: 'MELAMINE', panelColor: 'WHITE',
    backPanelThicknessMm: 8, backPanelEnabled: true, fronts: defaultFronts(), toeKickMm: 100,
...
export default function KitchenProject() {
  ...
  const load = async () => {
    if (!id) return
    const p = await kitchenProjectRepo.get(id)
  ...
  const saveProject = async (upd: KitchenProject) => {
    await kitchenProjectRepo.save(upd)
    setProject(upd)
  }
  ...
  const saveCabinet = async () => {
    if (!editing) return
    const now = Date.now()
    await cabinetRepo.save({ ...editing, updatedAt: now })
    setEditing(null)
    load()
  }

  const deleteCabinet = async (cid: string) => {
    if (!confirm(t('kitchen.confirmDeleteCabinet'))) return
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.tabs}>
        <button className={tab === 'cabinets' ? styles.activeTab : styles.tabBtn} onClick={() => setTab('cabinets')}>{t('kitchen.tabCabinets')}</button>
      ...
      {tab === 'cabinets' && (
        <div className={styles.cabinetsTab}>
          <div className={styles.actions}>
            <button className={styles.addBtn} onClick={openNew}>{t('kitchen.addCabinet')}</button>
            {templates.length > 0 && (
            ...
            {cabinets.map(c => (
              <div key={c.id} className={styles.cabinetCard} onClick={() => setPreview(c)}>
                ...
                <div className={styles.cabinetActions} onClick={e => e.stopPropagation()}>
                  <button onClick={() => openEdit(c)} className={styles.editBtn}>{t('kitchen.edit')}</button>
                  <button onClick={() => deleteCabinet(c.id)} className={styles.delBtn}>{t('kitchen.delete')}</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {editing && (
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            <h3>{editing.id ? t('kitchen.editCabinet') : t('kitchen.addCabinet')}</h3>
            ...
            <div className={styles.modalActions}>
              <button className={styles.saveBtn} onClick={saveCabinet}>{t('kitchen.save')}</button>
